...
for mail in cur.fetchall():
    ...
    for sent in cur.fetchall():
        ...
        product = sent[2]
        cve_id_list = []
        cur.execute("select DISTINCT cve_id from kb_checker.nvd_jsons join kb_checker.descriptions on kb_checker.nvd_jsons.id = kb_checker.descriptions.nvd_json_id where last_modified_date LIKE '" + d + "%' and value like '%" + product + "%'")
        for cve_id in cur.fetchall():
            cve_id_list.append(cve_id[0])
        cur.execute("select cve_id from jvns where last_modified_date LIKE '" + d + "%' and summary like '%" + product + "%'")
        for jvn_cve_id in cur.fetchall():
            if jvn_cve_id[0] in cve_id_list:
                cve_id_list.append(jvn_cve_id[0])
        for cve_id in cve_id_list:
            cur.execute("select title,jvn_link from jvns where cve_id = '" + cve_id + "'")
        ...
        system += "　" + vendor + " : " + product + " / " + str(severity_count) + "件\n"
